...
## Visão Geral
Este repositório contém exemplos de código Dart e Flutter, organizados em múltiplos diretórios, com foco em navegação aninhada, gerenciamento de rotas e testes de fontes customizadas.

## Estrutura Principal
- `lib/`: Código-fonte principal dos exemplos.
  - `nested-navigation/`: Demonstra navegação aninhada usando o pacote `auto_route`.
  - `mobile/`, `data/`, `declarative/`: Outras áreas de exemplos.
- `ios/`, `build/`: Artefatos de build e configuração para Flutter.

## Padrões e Convenções
- Navegação: Utilize o pacote `auto_route` para navegação declarativa. Rotas são definidas em classes como `NestedRouter` e anotadas com `@RoutePage`.
- Componentes: Telas são implementadas como `StatelessWidget` e usam `context.pushRoute` ou `AutoRouter.of(context).pushWidget` para navegação.
- Organização: Cada exemplo fica em seu próprio subdiretório dentro de `lib/`.
- Nomes de arquivos seguem o padrão kebab-case.

## Fluxos de Desenvolvimento
- **Build:**
  - Use `flutter pub get` para instalar dependências.
  - Use `flutter run` para executar exemplos.
- **Testes:**
  - Não há testes automatizados detectados; adicione arquivos em `test/` se necessário.
- **Debug:**
  - Use o VS Code ou Android Studio para depuração interativa.

## Integrações e Dependências
- `auto_route`: Gerencia rotas e navegação aninhada.
...
- Sempre siga os padrões de navegação e organização já presentes.
- Consulte `lib/nested-navigation/nested_navigation.router.dart` para exemplos de rotas e navegação.
- Mantenha o código modular e exemplos isolados por diretório.
