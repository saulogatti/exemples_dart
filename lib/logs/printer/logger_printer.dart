...
import 'package:exemplos_flutter/logs/printer/logger_notifier.dart';
...
final class LoggerPrinter {
  factory LoggerPrinter() {
    _logger ??= LoggerPrinter._private();
    return _logger!;
  }

  LoggerPrinter._private() {
  ...
  void logObject(LoggerObjectBase logger) {
    if (debugEnable || logger is ErrorLog) {
      ...
      final time = logger.getColor().call(logger.creationDateTime.onlyTime());
